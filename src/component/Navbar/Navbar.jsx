...
function Navbar() {
  ...
  useEffect(() => {
    const interval = setInterval(() => setToken(localStorage.getItem("token")), 500);
  ...
  return (
    <nav className="navbar">
      ...
      <div className="nav-right">
        ...
        {token ? (
          <button className="signin-btn" onClick={handleLogout}>{t("navbar.logout")}</button>
        ) : (
          <Link to="/auth/login" className="signin-btn">{t("navbar.login")}</Link>
        )}
      </div>
    </nav>
  );
